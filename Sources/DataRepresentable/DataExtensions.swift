...
//        let val = C.init(fromData: self)// self.withUnsafeBytes {$0.load(as: C.self)}
//        self = self.advanced(by: MemoryLayout<C>.stride)
//        if C.self is String.Type {
//            print("STRING")
...
//    public mutating func process<C: DataRepresentable>(count: Int) -> [C] {
